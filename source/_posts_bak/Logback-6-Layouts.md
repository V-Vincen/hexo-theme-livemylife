---
title: '[Logback] 6 Layout'
catalog: true
date: 2020-12-31 15:49:53
subtitle: Layout...
...
添加相应的 set 方法就可以开启属性的配置。`PrintThreadName` 属性是 `boolean` 而不是 `String` 类型。关于配置 logback 的详细信息请参见 [[Logback] 3 Logback 的配置](https://V-Vincen.github.io/2020/12/31/Logback-3-Logback%20%E7%9A%84%E9%85%8D%E7%BD%AE/)。[Chapter 11: Joran](http://logback.qos.ch/manual/onJoran.html) 将会提供更详细的内容。下面是关于 `MySampleLayout2` 的相关配置：
...
在 `PatternLayout` 中，括号用于对转换模式进行分组。**'(' 与 ')' 有特殊的含义，因此如果想用作字面量，需要进行特殊的转义**。圆括号的特殊含义将在 [下面](#特殊的圆括号) 进行详细的介绍。

之前提到过，特定的转换模式可以通过花括号指定可选的参数。一个简单的可选转换模式可以是 %logger{10}。在这里 "logger" 就是转换字符，10 就是可选参数。可选参将在 [下面](#转换字符的选项) 详细介绍。
...
| **property{key}**                                            | 输出属性 *key* 所对应的值。相关定义参见 [定义变量](https://V-Vincen.github.io/2020/12/31/Logback-3-Logback%20%E7%9A%84%E9%85%8D%E7%BD%AE/#%E5%8F%98%E9%87%8F%E6%9B%BF%E6%8D%A2) 以及 [作用域](https://V-Vincen.github.io/2020/12/31/Logback-3-Logback%20%E7%9A%84%E9%85%8D%E7%BD%AE/#%E4%BD%9C%E7%94%A8%E5%9F%9F)。如果 key 在 logger context 中没有找到，那么将会去系统属性中找。<br />*key* 没有默认值，如果缺失，则会展示 " Property_HAS_NO_KEY" 的错误信息。 |
...
除了可以输出 TRACE, DEBUG, WARN, INFO 或者 ERROR 来表示日志等级之外，还是输出T, D, W, I 与 E 来进行表示。你可以 [自定义转换器](#自定义转换说明符) 或者利用刚才讨论的格式修改器来缩短日志级别为一个字符。这个转换说明符可能为 "%.-1level"。
...
如上所述的 [圆括号](#特殊的圆括号) 分组，允许对子模式进行着色。在 1.0.5 版本，`PatternLayout` 可以识别 "%black"，"%red"，"%green"，"%yellow"，"%blue"，"%magenta","%cyan", "%white", "%gray", "%boldRed","%boldGreen", "%boldYellow", "%boldBlue", "%boldMagenta""%boldCyan", "%boldWhite" 以及 "%highlight" 作为转换字符。这些转换字符都还可以包含一个子模式。任何被颜色转换字符包裹的子模式都会通过指定的颜色输出。
...
Evaluator 与 *评价表达式 (evaluation expressions)* 都会在 [[Logback] 7 Filters](https://V-Vincen.github.io/2020/12/31/Logback-7-Filters/) 详细介绍。如果你想利用 evaluator 去做一些有意思的事情，你必须看一下对这个的详细介绍。下面的例子基于 `JaninoEventEvaluator`，所以需要 [Janino 类库](http://docs.codehaus.org/display/JANINO/Home)。查看 [相关文档](https://logback.qos.ch/setup.html#janino) 进行设置。
...
<configuration>
    ...
    <appender name="STDOUT" class="ch.qos.logback.core.ConsoleAppender">
        <encoder>
            <pattern>%-4relative [%thread] %-5level - %msg%n%caller{2, DISP_CALLER_EVAL}</pattern>
        </encoder>
    </appender>
    <root level="DEBUG">
        <appender-ref ref="STDOUT" />
...
第二条日志没有堆栈信息，因为我们禁止 `TextException` 类型的堆栈信息。每条堆栈信息的最后用综括号包裹起来的是具体的 [包信息](#xThrowable)。
...
![htmlLayoutAccess](htmlLayoutAccess.gif)
